...
class NetworkConnectivityImpl (
    ...
    override fun observeNetworkStatus(): Flow<Result<NetworkStatus>> = callbackFlow {
    ...
    private fun NetworkCapabilities.toNetworkStatus(): NetworkStatus {
        return when {
            hasTransport(NetworkCapabilities.TRANSPORT_WIFI) -> NetworkStatus.Connected(
                NetworkStatus.NetworkType.Wifi
            )
            hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) -> NetworkStatus.Connected(
